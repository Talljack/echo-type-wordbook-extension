# EchoType Wordbook Privacy Policy

Last updated: September 19, 2026

EchoType Wordbook helps users save English words encountered on webpages and turn them into study cards. This policy explains what data the extension handles and why.
...
- API keys are stored locally and are sent only to the provider endpoint configured by the user. EchoType Wordbook does not operate an intermediary server and does not receive those keys.

## Data sharing and sale

EchoType Wordbook does not sell user data, use it for advertising, or share it with data brokers. Data is transmitted only when necessary to provide the dictionary, translation, or user-configured AI features described above.
...
- `contextMenus`: Provide the “Add to EchoType Wordbook” action for selected text.
...
EchoType 网页生词本用于收藏用户在网页中遇到的英文单词，并生成可学习的词卡。本政策说明扩展会处理哪些数据以及处理目的。
...
- API Key 仅保存在本机，并且只发送给用户配置的服务商接口。EchoType 不运营中转服务器，也不会接收这些密钥。

## 数据共享与出售

EchoType 不出售用户数据，不将数据用于广告，也不向数据经纪商共享数据。扩展只会在提供上述词典、翻译或用户配置的 AI 功能所必需时传输数据。
...
- `contextMenus`：为选中的文本提供“加入 EchoType 词书”菜单。
