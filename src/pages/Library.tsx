      ...
      <div className="library-grid">
        ...
        <section className="detail-pane">
          {selected ? <>
            ...
            <label className="field"><span>中文释义</span><input value={selected.translation} onChange={(event) => setWords((items) => items.map((item) => item.id === selected.id ? { ...item, translation: event.target.value } : item))} onBlur={(event) => void patchWord({ translation: event.target.value })} /></label>
            {!!selected.definitions.length && <DetailSection title="英文释义">{selected.definitions.map((item, index) => <p key={item}><b>{index + 1}.</b> {item}</p>)}</DetailSection>}
